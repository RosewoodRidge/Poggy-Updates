...
2. **Place** the `poggy_multijob` folder in your server's `resources` directory
   ```
   resources/
   └── [jobs]/
       └── poggy_multijob/
...
### Upgrading from the `multijob` folder

From 1.6.0 the resource folder is called `poggy_multijob` (it used to be `multijob`).

1. Copy your settings out of the old `shared/config.lua` and `shared/locales.lua`.
2. Delete the old `multijob` folder and put `poggy_multijob` in its place.
3. In server.cfg, change `ensure multijob` to `ensure poggy_multijob`.
4. Put your settings back into `config.lua` and `translations.lua` (both now sit next to `fxmanifest.lua`).

Your players' jobs stay where they are: the table is still `poggy_multijob`, and
`/multijob`, `/mj` and `/mjadmin` are unchanged. If one of your own resources calls
`exports.multijob:GetJobs()` or listens for a `multijob:` event, change it to
`exports.poggy_multijob:GetJobs()` and `poggy_multijob:`.

There is no SQL to import: poggy_core creates the `poggy_multijob` table
automatically when poggy_multijob starts. **Upgrading from 1.3.0 or earlier?** Your
rows are copied out of `marshal_multi_jobs` automatically on the first start. The
old table is kept so you can roll back; delete it yourself once you are happy.
...
### Job Presets Tab
- **Categorized Presets** - Police, Medical, Business, Shops
...
Edit the `Config.JobPresets` section in `config.lua`:
...
Edit `translations.lua` to customize all text strings:
...
- The `poggy_multijob` table is created when poggy_multijob starts; look for a red `poggy_multijob database:` line in the server console
...
For issues and feature requests, join the Poggy Scripts Discord: https://discord.com/invite/rBarFeuzFj

## Credits

- Original concept by Marshal
- Modernized and enhanced by Poggy
